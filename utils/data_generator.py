...
class Generator(object):
    ...
    def preprocess(self):
        ...
        assign_source = np.array([np.array(df)[i:i + self.time_seq] for i in range(len(df) - self.time_seq - 2)])
        assign_target = np.array([np.array(df)[i + self.time_seq: i + self.time_seq + 3]
                                  for i in range(len(df) - self.time_seq - 2)])
